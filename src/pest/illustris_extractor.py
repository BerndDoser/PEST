...
from pest.extractor import Extractor
...
class IllustrisExtractor(Extractor):
    ...
    def extract(self) -> Dict[str, Any]:
        ...
        base_path = str(self.simulation_path / "output")

        # Load simulation header
        header = il.groupcat.loadHeader(base_path, self.snapshot)
        ...
        subhalos = il.groupcat.loadSubhalos(base_path, self.snapshot)
                ...
                for field in fields:
                    try:
                        field_data = self._load_particle_field(
                            il, base_path, self.snapshot, subhalo_id, ptype, field, mass_units_msun, dist_units_kpc
                        )
    ...
    def validate_configuration(self) -> bool:
        ...
        if not self.simulation_path.exists():
            print(f"Error: Simulation path does not exist: {self.simulation_path}")
            return False

        # Check if objects type is valid
        if self.objects not in self.OBJECT_TYPES:
            print(f"Error: Invalid objects type '{self.objects}'. Must be one of {self.OBJECT_TYPES}")
            return False

        # Validate components configuration
        for component_config in self.components:
            if not isinstance(component_config, dict):
                print("Error: Each component must be a dictionary")
                return False

            if "name" not in component_config:
                print("Error: Component must have a 'name' field")
                return False

            component_name = component_config["name"]
            if component_name not in self.AVAILABLE_FIELDS:
                print(f"Error: Unknown component '{component_name}'. Available: {list(self.AVAILABLE_FIELDS.keys())}")
                return False

            if "fields" not in component_config:
                print("Error: Component must have a 'fields' field")
                return False
    ...
    def _load_particle_field(
        ...
        # Load particle data for this subhalo
        particle_data = il.snapshot.loadSubhalo(base_path, snapshot, subhalo_id, ptype, fields=[illustris_field])
